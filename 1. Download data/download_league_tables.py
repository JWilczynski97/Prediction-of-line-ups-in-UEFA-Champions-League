...
# The task of this script was downloading websites with european leagues table as html files.
# The program uses the data about teams and leagues from local database.
# After downloading of every table the data are saving into the local database with league tables.
...
def download_websites(select_season, select_league):
    for match in database.select("Matches", order_by="Date", Season=select_season, League=select_league):
        ...
        if path.exists(file):
            ...
            continue
        if browser.current_url != "https://www.footstats.co.uk/index.cfm?task=league_full":
            browser.get('https://www.footstats.co.uk/index.cfm?task=league_full')
            wait(2, 3)
...
db_tables = Database("../Databases/League_Tables.db", logger=log)
...
for season, league in product(ALL_SEASONS, list(LEAGUES.keys()) + ["Russia"]):
    ...
    if league == "Russia":
        ...
        continue
    # download websites with league tables from www.footstats.co.uk
