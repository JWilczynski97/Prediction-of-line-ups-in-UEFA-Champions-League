...
# The goal of this program was an analysis of data from earlier downloaded websites. Data from local databases were used too.
# The program analysed all data and determine new necessary data. Destination data is table Performances in the local database collecting data
# about performances of every player in the UEFA Champions League matches from all analysed teams.
# This table are used as learning and testing dataset for the machine learning algorithms in the next steps of this project.
# Used classes: Match, PrevMatch, Team, Performance, PrevPerformance, Incident.
...
LOGS_DIRECTORY = r"../Logs/save_performances_data_to_db"
DB_PATH = "../Databases/League_Tables.db"
logger = Logger(log_folder=LOGS_DIRECTORY, std_output=True)
database = Database(LOGS_DIRECTORY, logger)
league_tables = Database(DB_PATH, logger)


class Match:
    """Object of this class represents a match of UEFA Champions League analysed in the project.
    These data are getting from a table UCL_Matches of local database."""
    matches = dict()
    logger = logger
...
class PrevMatch:
    """Object of this class represents a previous match, so the league game preceding an analysed UEFA Champions League match.
    These data are getting from the table Matches of local database."""
    prev_matches = dict()
    logger = logger
    ...
    def __init__(self, prev_match_id, ucl_match, team, competition="league"):
        ...
        # creating of team objects (depending of the match type)
        self.team_home = Team(match_data["Team_1_ID"], "home", self.match_id, prev=True) if str(
            match_data["Team_1_ID"]) == self.team_whoscored_id else str(match_data["Team_1_ID"])
        self.team_away = Team(match_data["Team_2_ID"], "away", self.match_id, prev=True) if str(
            match_data["Team_2_ID"]) == self.team_whoscored_id else str(match_data["Team_2_ID"])
...
class Team:
    """Object of this class represents one of both teams from analysed match (from UEFA Champions League match or other league).
    These data are saving into the tables Teams_in_matches and Teams_of_ucl_matches of local database."""
    teams = dict()
    logger = logger
    ...
    def __init__(self, whoscored_id, kind, match_id, prev=False):  # kind is a value from ("home", "away")
        ...
        try:
            ...
            logger.write(f"Team -> {self.team_id} -> AssertionError len(self.predicted) == 11 and len(self.starting) == 11. Content: {e}",
                level=logging.ERROR)
        ...
        if self.prev is False:
            ...
            for num, prev_ucl_match_id in self.prev_ucl_matches.copy().items():
                self.prev_ucl_matches[num] = PrevMatch(str(prev_ucl_match_id), ucl_match=self.match_id, team=str(self.team_id),
                    competition='ucl').team
...
class Incident:
    """Object of this class represents an incident (for example yellow card or input of substitute) in an analysed match."""
...
class Performance:
    """Object of this class represents data about (un)possible performance of player in analysed UCL match.
    These data are saving into the table PRevPerformances of local database.
    These data are destination data and are used as dataset for machine learning algorithm in the next steps od project."""
    players = dict()
    logger = logger
...
class Prev_Performance:
    """Object of this class represents data about performance of player in one of previous league match before analysed UCL match.
    These data are saving into the table PRevPerformances of local database."""
    prev_players = dict()
    logger = logger
    ...
    def analyze_incidents(self):
        ...
        if self.substitute == 1:
            try:
                ...
                warnings.warn(
                    f"{self.prev_performance_id} -> AssertionError assert self.substitute == 1 and data['sub in'] is not None. Content: {e}")
                warnings.warn(f"data: {data}. substitute: {self.substitute}")
        return data
...
for game in database.select("UCL_Matches", order_by="Date"):
    ...
    logger.write(70 * "-")
logger.write("The end of data analysis.")
database.disconnect()
